...
def test_prometheus_config() -> None:
    ...
    assert any("/etc/prometheus/rules/" in str(p) for p in (rule_files or [])), (
        "Expected rule_files to include /etc/prometheus/rules/*.yml, "
        f"got: {rule_files}"
    )
    ...
    assert any(t == "alertmanager:9093" for t in targets), (
        "Expected alertmanager target 'alertmanager:9093' in targets, "
        f"got: {targets}"
    )


def test_prometheus_rules_content() -> None:
    ...
    assert any(g.get("name") == "fado_crm_alerts" for g in groups), (
        "Missing group 'fado_crm_alerts'"
    )
    ...
    assert not missing, (
        f"Missing expected alerts: {sorted(missing)}; found={sorted(found)}"
    )

    # Basic severity checks for present alerts
    for name, sev in severities.items():
        assert sev in {"warning", "critical"}, (
            f"Unexpected severity for {name}: {sev}"
        )


def test_alertmanager_config() -> None:
    ...
    assert route.get("receiver") == "devnull", (
        f"Expected default receiver 'devnull', got {route.get('receiver')}"
    )

    receivers = data.get("receivers", []) or []
    names = {r.get("name") for r in receivers if isinstance(r, dict)}
    assert "devnull" in names, (
        f"Expected a 'devnull' receiver in receivers, got {names}"
    )
