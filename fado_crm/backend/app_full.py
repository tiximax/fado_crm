...
async def upload_product_image_endpoint(
    ...
    try:
        ...
        with open(out_path, 'wb') as f:
            f.write(content)
        return {"success": True, "message": "Upload hinh anh thanh cong", "data": {"file_info": {"stored_filename": stored}, "url": f"/uploads/product_images/{stored}"}}
    except HTTPException:
        raise
    except Exception as e:
...
async def upload_multiple_images_endpoint(
    ...
    try:
        ...
        return {"success": True, "message": f"Upload hoan thanh: {success_count} thanh cong, {fail_count} that bai", "data": {"results": results}}
    except HTTPException:
        raise
    except Exception as e:
        ...
        if _fs is not None:
            ...
            if category == 'product_images':
                for p in (Path('uploads') / 'product_images').glob('*'):
                    if p.is_file():
                        items.append({"filename": p.name, "url": f"{base}/uploads/product_images/{p.name}"})
