...
async def get_khach_hang(khach_hang_id: int, db: Session = Depends(get_db)):
    ...
    if not khach_hang:
        raise HTTPException(
            ...
            detail="Khong tim thay khach hang"
        )
    return khach_hang
...
async def create_san_pham(
    san_pham: schemas.SanPhamCreate,
    db: Session = Depends(get_db)
):
    """Tao san pham moi"""
...
async def create_don_hang(
    don_hang: schemas.DonHangCreate,
    db: Session = Depends(get_db)
):
    """Tao don hang moi"""
    ...
    db_don_hang = DonHang(
        **don_hang.dict(),
        ...
        trang_thai=TrangThaiDonHang.CHO_XAC_NHAN
    )
    db.add(db_don_hang)
    db.commit()
...
async def update_don_hang_status(
    don_hang_id: int,
    trang_thai_update: schemas.TrangThaiUpdate,
    db: Session = Depends(get_db)
):
